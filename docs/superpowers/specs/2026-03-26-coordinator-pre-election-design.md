...
initCoordinatorContext()                          ← 加载所有数据，liveSet 完整
  ↓
【新增】preElectLeadersForLeaderlessBuckets()     ← 预选举
  ↓
...
```java
private void preElectLeadersForLeaderlessBuckets() {
...
Coordinator 重启
  ...
  → preElectLeadersForLeaderlessBuckets():
...
| 文件 | 变更 |
|------|------|
| `CoordinatorEventProcessor.java` | 新增 `preElectLeadersForLeaderlessBuckets()` 方法，在 `startup()` 中调用 |
