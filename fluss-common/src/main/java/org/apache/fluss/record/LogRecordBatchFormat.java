...
public class LogRecordBatchFormat {
    ...
    private static final int EXTEND_PROPERTIES_LENGTH_LENGTH = 4;
    ...
    private static final int V3_EXTEND_PROPERTIES_LENGTH_OFFSET =
            V3_RECORDS_COUNT_OFFSET + RECORDS_COUNT_LENGTH;
    public static final int V3_BASE_RECORD_BATCH_HEADER_SIZE =
            V3_EXTEND_PROPERTIES_LENGTH_OFFSET + EXTEND_PROPERTIES_LENGTH_LENGTH;
    ...
    /**
     * Returns the offset of the extend properties length field for V3 format.
     *
     * @param magic the magic value of the record batch
     * @return the offset of extend properties length field
     * @throws IllegalArgumentException if the magic value is not V3
     */
    public static int extendPropertiesLengthOffset(byte magic) {
        if (magic == LOG_MAGIC_VALUE_V3) {
            return V3_EXTEND_PROPERTIES_LENGTH_OFFSET;
        }
        throw new IllegalArgumentException(
                "extendPropertiesLengthOffset is only available for V3 format");
    }

    /**
     * Returns the offset of the extend properties data field for V3 format.
     *
     * @param magic the magic value of the record batch
     * @return the offset of extend properties data field
     * @throws IllegalArgumentException if the magic value is not V3
     */
    public static int extendPropertiesDataOffset(byte magic) {
        if (magic == LOG_MAGIC_VALUE_V3) {
            return V3_EXTEND_PROPERTIES_LENGTH_OFFSET + EXTEND_PROPERTIES_LENGTH_LENGTH;
        }
        throw new IllegalArgumentException(
                "extendPropertiesDataOffset is only available for V3 format");
    }
}
