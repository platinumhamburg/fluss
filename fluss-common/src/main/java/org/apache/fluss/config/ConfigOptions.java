...
public class ConfigOptions {
    ...
    public static final ConfigOption<Integer> SERVER_INDEX_CACHE_PENDING_WRITE_THREADS =
    ...
    public static final ConfigOption<Integer> INDEX_REPLICA_FETCH_MIN_ADVANCE_STEP =
            key("index.replica.fetch.min-advance-step")
                    .intType()
                    .defaultValue(512)
                    .withDescription(
                            "The minimum number of offsets to advance for index fetch requests when the current offset "
            ...
            key("table.type")
                    ...
                    .withDescription(
                            "The type of the table. The default value is `DATA_TABLE`. "
                                    + "The supported types are `DATA_TABLE` and `INDEX_TABLE`.");
    ...
    public static final ConfigOption<Duration> TABLE_KV_COMPACTION_FILTER_TTL_RETENTION =
            key("table.kv.compaction-filter.ttl.retention")
                    .durationType()
                    .defaultValue(Duration.ofMillis(-1))
                    .withDescription(
                            "The TTL retention for KV values when using TTL compaction filter. "
