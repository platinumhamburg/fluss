...
/**
 * A decoder to decode a timestamp, schema id and {@link BinaryRow} from a byte array value which is
 * encoded by {@link TsValueEncoder#encodeValue(long, short, BinaryRow)}.
 *
 * <p>This decoder supports schema evolution by dynamically creating RowDecoders based on the schema
 * id embedded in the value bytes. It is similar to {@link ValueDecoder} but with a timestamp prefix
...
public class TsValueDecoder {
    ...
    public TsValueDecoder(SchemaGetter schemaGetter, KvFormat kvFormat) {
    ...
    /**
     * Decode the value bytes and return the timestamp, schema id and the row encoded in the value
     * bytes.
     */
    public TsValue decodeValue(byte[] valueBytes) {
        ...
        BinaryRow row =
                rowDecoder.decode(
                        ...
                        valueBytes.length - TS_LENGTH - SCHEMA_ID_LENGTH);
        return new TsValue(ts, schemaId, row);
    }

    /**
     * The timestamp, schema id and {@link BinaryRow} stored as the value of kv store.
     *
