...
public class LazyMemorySegmentPool implements MemorySegmentPool, Closeable {
    ...
    LazyMemorySegmentPool(
        ...
        this.perRequestPages = Math.max(1, Math.toIntExact(perRequestMemorySize / pageSize()));
    ...
    public static LazyMemorySegmentPool createIndexCacheBufferPool(Configuration conf) {
        ...
        int segmentCount = (int) (totalBytes / pageSize);
        long waitTimeout = conf.get(ConfigOptions.SERVER_INDEX_CACHE_WAIT_TIMEOUT).toMillis();
        return new LazyMemorySegmentPool(segmentCount, pageSize, waitTimeout, perRequestMemorySize);
    }
