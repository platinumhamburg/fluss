...
public final class IndexTableUtils {
    ...
    public static TableDescriptor createIndexTableDescriptor(
        ...
        // if main table columns have columnId, we need to assign columnId to __offset as well
        boolean mainColumnsHaveColumnId =
                !indexTableColumns.isEmpty()
                        && indexTableColumns.get(0).getColumnId()
                                != Schema.Column.UNKNOWN_COLUMN_ID;
        if (mainColumnsHaveColumnId) {
            // find the max columnId and assign the next one to __offset
            int maxColumnId =
            ...
            indexTableColumns.add(
                    new Schema.Column("__offset", DataTypes.BIGINT(), null, maxColumnId + 1));
        } else {
