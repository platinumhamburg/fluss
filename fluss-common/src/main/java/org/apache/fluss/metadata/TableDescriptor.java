...
public final class TableDescriptor implements Serializable {
    ...
    public static TableDescriptor forIndexTable(
        ...
        // Add source tracking columns for index replication state.
        // These columns record the origin of each index row (which partition/bucket/offset
        // it came from), enabling state recovery from the index data itself.
        {
            int nextColumnId =
                    indexTableColumns.stream()
                                    .mapToInt(Schema.Column::getColumnId)
                                    ...
                                    .orElse(Schema.Column.UNKNOWN_COLUMN_ID)
                            + 1;
            // source_partition_id is nullable (null for non-partitioned tables)
            indexTableColumns.add(
                    new Schema.Column(
