...
public class NullableSerializer<T> implements Serializer<T> {
    ...
    @Override
    public void serialize(T record, OutputView target) throws IOException {
        if (record == null) {
    ...
    @Override
    public T deserialize(InputView source) throws IOException {
        boolean isNull = deserializeNull(source);
        return isNull ? null : originalSerializer.deserialize(source);
    }

    private boolean deserializeNull(InputView source) throws IOException {
