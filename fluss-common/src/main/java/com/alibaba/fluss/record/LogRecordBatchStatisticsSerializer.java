...
public class LogRecordBatchStatisticsSerializer {
    ...
    public static byte[] serialize(LogRecordBatchStatistics statistics, RowType rowType)
        ...
        ByteBuffer buffer = ByteBuffer.allocate(estimateSerializedSize(statistics, rowType));
        OutputView outputView = new DataOutputSerializer(buffer.capacity());
        ...
        for (int i = 0; i < fieldCount; i++) {
            ...
            if (minValues != null && !minValues.isNullAt(i)) {
                ...
                writeValue(outputView, minValues, i, fieldType);
            } else {
            ...
            if (maxValues != null && !maxValues.isNullAt(i)) {
                outputView.writeBoolean(true);
                writeValue(outputView, maxValues, i, fieldType);
            } else {
                outputView.writeBoolean(false);
            }
        }
    ...
    public static LogRecordBatchStatistics deserialize(byte[] data, RowType rowType)
        ...
        for (long nullCount : nullCounts) {
            totalRowCount += nullCount;
        }

        return new DefaultLogRecordBatchStatistics(
                totalRowCount,
                GenericRow.of(minValues),
                GenericRow.of(maxValues),
