...
public class LogRecordBatchFormat {
     ...
     *   <li>Statistics => [StatisticsData] (New in V2, optional based on StatisticsLength)
     *   <li>Records => [Record]
     * </ul>
     *
     ...
     * is 0, no statistics are present. The Statistics data contains serialized column-level
     * statistics.
     *
     * <p>The current attributes are given below:
     *
    ...
    private static final int V2_STATISTICS_OFFSET =
            V2_STATISTICS_LENGTH_OFFSET + STATISTICS_LENGTH_LENGTH;

    // V2 records offset is dynamic based on statistics length
    private static final int V2_RECORD_BATCH_HEADER_SIZE_WITHOUT_STATS = V2_STATISTICS_OFFSET;
    ...
    public static int commitTimestampOffset(byte magic) {
        switch (magic) {
            ...
            case LOG_MAGIC_VALUE_V2:
                return V2_COMMIT_TIMESTAMP_OFFSET;
            default:
    ...
    public static int recordBatchHeaderSize(byte magic) {
        switch (magic) {
            ...
            case LOG_MAGIC_VALUE_V2:
                // For V2, header size depends on statistics length, use minimal size
                return V2_RECORD_BATCH_HEADER_SIZE_WITHOUT_STATS;
            default:
                throw new IllegalArgumentException("Unsupported magic value " + magic);
        }
    }
    ...
    public static int recordBatchHeaderSizeWithStats(byte magic, int statisticsLength) {
        switch (magic) {
            ...
            case LOG_MAGIC_VALUE_V1:
                return V1_RECORD_BATCH_HEADER_SIZE;
            case LOG_MAGIC_VALUE_V2:
                return V2_RECORD_BATCH_HEADER_SIZE_WITHOUT_STATS + statisticsLength;
            default:
                throw new IllegalArgumentException("Unsupported magic value " + magic);
        }
    }
    ...
    public static int arrowChangeTypeOffset(byte magic) {
        switch (magic) {
            ...
            case LOG_MAGIC_VALUE_V2:
                // For V2, this depends on statistics length, use minimal offset
                return V2_RECORD_BATCH_HEADER_SIZE_WITHOUT_STATS;
            default:
                throw new IllegalArgumentException("Unsupported magic value " + magic);
        }
