...
public class DefaultLogRecordBatch implements LogRecordBatch {
    ...
    public Optional<LogRecordBatchStatistics> getStatistics() {
        ...
        try {
            ...
            // We need the row type to deserialize statistics, but we don't have it here
            // For now, return empty. The statistics will be available through ReadContext
            // TODO: Consider passing RowType to this method or caching it
            return Optional.empty();
        } catch (Exception e) {
            // If reading statistics fails, return empty
            return Optional.empty();
        }
    }
