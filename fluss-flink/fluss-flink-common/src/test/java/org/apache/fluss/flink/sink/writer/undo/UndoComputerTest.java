...
class UndoComputerTest {
    ...
    void testChangeTypeToUndoMapping(
        ...
        assertThat(mockWriter.deleteCount).isEqualTo(expectDelete ? 1 : 0);
        assertThat(mockWriter.upsertCount).isEqualTo(expectUpsert ? 1 : 0);

        if (expectDelete) {
            assertThat(mockWriter.lastDeletedRow).isSameAs(testRow);
        }
        if (expectUpsert) {
            assertThat(mockWriter.lastUpsertedRow).isSameAs(testRow);
        }
    }
    ...
    void testPrimaryKeyDeduplication() {
        ...
        assertThat(mockWriter.deleteCount).isEqualTo(2);
        assertThat(mockWriter.upsertCount).isEqualTo(0);

        // Verify: correct rows were processed
        assertThat(mockWriter.allDeletedRows).containsExactly(row1, row3);
    }
    ...
    void testUpdateAfterDoesNotBlockSubsequentRecords() {
        ...
        assertThat(mockWriter.deleteCount).isEqualTo(1);
        assertThat(processedKeys).hasSize(1);
    }
}
