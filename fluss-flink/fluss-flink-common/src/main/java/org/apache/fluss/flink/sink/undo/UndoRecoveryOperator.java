...
public class UndoRecoveryOperator<IN> extends AbstractStreamOperator<IN>
     ...
     *
     * <p>This ID combines the bridge ID from {@link UndoRecoveryOperatorFactory} with this
     * operator's runtime subtask index. It is used by {@link #close()} to remove this operator from
     * the registry by ID (O(1)) instead of by reference (O(n)).
     */
     ...
     * @param offsetReporterBridgeId the stable bridge ID shared with the downstream sink
     */
    public UndoRecoveryOperator(
        ...
        this.offsetReporterRegistryId =
                UndoRecoveryOperatorFactory.createRegistryId(
