...
public class LookupNormalizer implements Serializable {
        ...
        if (!supportPrefixLookup) {
            ...
            throw new TableException(
                    "The Fluss lookup function supports lookup tables where the lookup keys "
                            + "include all primary keys or match a secondary index, the primary keys are "
                            + primaryKeyNames
    ...
    private static int findIndex(List<String> columnNames, String key) {
        ...
        throw new TableException(
                "The Fluss lookup function supports lookup tables where the lookup keys include all primary keys, all bucket keys, or match a secondary index."
    ...
    private static boolean matchesSecondaryIndex(int[] lookupKeyIndexes, int[][] secondaryIndexes) {
        ...
        for (int[] indexDef : secondaryIndexes) {
            // Require exact column order match for composite index correctness.
            // This must be consistent with TableLookup.findMatchingSecondaryIndex()
            // which uses List.equals() (order-sensitive).
            if (Arrays.equals(lookupKeyIndexes, indexDef)) {
                return true;
            }
        }
