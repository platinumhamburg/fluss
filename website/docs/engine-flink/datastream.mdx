...
* Support for filter pushdown on Arrow log tables
...
* **setFilter(Predicate filter):** Sets a predicate for server-side record batch filtering
...
#### Using Filter Pushdown
Filter pushdown uses column statistics to skip record batches that cannot contain matching rows.
Build a Fluss predicate from the table's `RowType` and pass it to `FlussSourceBuilder#setFilter`:

```java
PredicateBuilder predicateBuilder = new PredicateBuilder(orderRowType);
int amountIndex = predicateBuilder.indexOf("amount");
Predicate predicate = predicateBuilder.greaterOrEqual(amountIndex, 100);
...
DataStream<Order> orders = env.fromSource(
        flussSource,
        WatermarkStrategy.noWatermarks(),
        "Filtered Fluss Source")
    // Batch-level pushdown can produce false positives, so retain the exact filter.
    .filter(order -> order.getAmount() >= 100);
```

In this example, `orderRowType` is the Fluss table's `RowType`. Filter pushdown is available only
for Log Tables using the Arrow log format, and every filtered column must be configured in the
table's `table.statistics.columns` property. See
[Filter Pushdown](reads.md#filter-pushdown) for the supported predicates and configuration details.
