...
For a bugfix release you need to update the version in these files. For example, if you are releasing `0.8.1`, replace the previous version `0.8.0` with `0.8.1` in those files.
...
For example, if the next major version is `0.9.0`, you should replace all the previous version `0.8.0` to the next release version `0.9.0`.
...
Example output for release version `0.8.0` and RC number `1`:

```bash
tools $ tree releasing/release
releasing/release
├── fluss-0.8.0-bin.tgz
├── fluss-0.8.0-bin.tgz.asc
├── fluss-0.8.0-bin.tgz.sha512
├── fluss-0.8.0-src.tgz
├── fluss-0.8.0-src.tgz.asc
├── fluss-0.8.0-src.tgz.sha512
└── helm-chart
    ├── 0.8.0-rc1
    │     ├── fluss-0.8.0.tgz
    │     ├── fluss-0.8.0.tgz.asc
    │     ├── fluss-0.8.0.tgz.prov
    │     ├── fluss-0.8.0.tgz.prov.asc
    │     ├── fluss-0.8.0.tgz.prov.sha512
    │     └── fluss-0.8.0.tgz.sha512
    └── index.yaml
...
Then, update the `dockerVersion` field for the current release RC version in the `website/fluss-versions.json` file on the `main` branch to the value of `${RELEASE_VERSION}_rc${RC_NUM}` (which includes the RC suffix, e.g., `0.8.0-rc1`). This update should be committed and pushed to the `main` branch.
...
Subject: [VOTE] Release Fluss 0.8.0 (RC1)

Hi everyone,

Please review and vote on the release candidate #1 for the Apache Fluss version 0.8.0, as follows:
...
(NB: you have to build the Docker images locally with the version 0.8.0 in order to test Helm charts)
...
* https://github.com/apache/fluss/releases/tag/v0.8.0-rc1
...
From: Release Manager
To: dev@fluss.apache.org
Subject: [RESULT][VOTE] Release Fluss 0.8.0 (RC1)
...
Subject: [ANNOUNCE] Apache Fluss 0.8.0 released

The Apache Fluss community is very happy to announce the release of Apache Fluss 0.8.0.
...
https://downloads.apache.org/fluss/fluss-0.8.0/

Maven artifacts for Fluss can be found at:

https://search.maven.org/search?q=g:org.apache.fluss%20AND%20v:0.8.0
