...
class PartitionTombstoneMetadataPropagationTest {
    ...
    @Test
    void testStaleTombstoneDoesNotOverrideNewerVersion() {
        TabletServerMetadataCache cache = new TabletServerMetadataCache(new EmptyMetadataManager());

        PartitionTombstone newer = new PartitionTombstone(20L, asSet(99L), 2L);
        cache.updateClusterMetadata(
                new ClusterMetadata(
                        null,
                        new HashSet<>(),
                        Collections.emptyList(),
                        Collections.emptyList(),
                        Collections.singletonMap(TABLE_A, newer)));

        PartitionTombstone stale = new PartitionTombstone(0L, Collections.emptySet(), 1L);
        cache.updateClusterMetadata(
                new ClusterMetadata(
                        null,
                        new HashSet<>(),
