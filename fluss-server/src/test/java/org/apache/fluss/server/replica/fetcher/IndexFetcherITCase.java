...
import org.apache.fluss.server.index.IndexBucketCacheManager;
...
public class IndexFetcherITCase {
    ...
    void testIndexFetcherErrorHandlingAndRetry() throws Exception {
        ...
        TablePath indexTablePath = TablePath.forIndexTable(INDEXED_TABLE_PATH, "idx_name");
        ...
        retry(
                ...
                () -> {
                    ...
                    for (int i = 0; i < INDEXED_DATA_WITH_KEY_AND_VALUE.size(); i++) {
                        ...
                        // Index table row format: (indexColumn, primaryKey)
                        // For name index: (name, id)
    ...
    private void assertIndexLookupResponse(
        ...
        // Index tables use ValueEncoder (not TsValueEncoder) when main table has no TTL
        // (i.e., non-partitioned tables or partitioned tables without retention)
        ...
        LOG.debug(
                "Index lookup response verified: schemaId={}, row={}",
                schemaId,
                    ...
                    for (int serverId = 0;
                        ...
                        try {
                            ...
                            List<Replica> onlineReplicas =
                                    replicaManager.getOnlineReplicaListForTest();

                            // Check IndexDataProducer statistics from all replicas
                            int totalCacheEntries = 0;
                            int totalMemorySegments = 0;

                            for (Replica replica : onlineReplicas) {
                                if (replica.getIndexDataProducer() != null) {
                                    IndexBucketCacheManager cacheManager =
                                            replica.getIndexDataProducer().getCacheManager();
                                    if (cacheManager != null) {
                                        totalCacheEntries += cacheManager.getTotalEntries();
                                        totalMemorySegments +=
                                                cacheManager.getTotalMemorySegmentCount();
                                    }
                                }
                            }

                            LOG.info(
                                    "TabletServer {} IndexDataProducer Statistics: "
                                            + "Total entries: {}, Total memory segments: {}",
                                    serverId,
                                    totalCacheEntries,
                                    totalMemorySegments);

                            // Verify IndexDataProducer holds minimal memory
                            // After replication completion, cache should be mostly empty
                            assertThat(totalMemorySegments)
                                    .as(
                                            "IndexDataProducer should hold minimal memory segments after GC for server %d",
                                            serverId)
                            ...
                            if (totalCacheEntries > 0) {
                                assertThat(totalCacheEntries)
                                        .as(
                                                "IndexDataProducer entries should be minimal after GC for server %d",
                                                serverId)
                                        .isLessThanOrEqualTo(
    ...
    void testPartitionedTableIndexTimestampAssignment() throws Exception {
        ...
        // Using format "yyyy" to match table.auto-partition.time-unit=year
        String partition2023 = "2023";
        String partition2024 = "2024";
        ...
        // Step 4.5: Wait for data table high watermark to advance
        ...
        retry(
                Duration.ofMinutes(1),
                () -> {
                    long hw2023 = dataReplica2023.getLogHighWatermark();
                    long hw2024 = dataReplica2024.getLogHighWatermark();
                    assertThat(hw2023)
                            .withFailMessage(
                                    "2023 partition high watermark should be at least 2, current: %d",
                                    hw2023)
                            .isGreaterThanOrEqualTo(2);
                    assertThat(hw2024)
                            .withFailMessage(
                                    "2024 partition high watermark should be at least 2, current: %d",
                                    hw2024)
                            .isGreaterThanOrEqualTo(2);
                });

        LOG.info(
                "Step 4.5 completed: Data table high watermarks advanced - 2023: {}, 2024: {}",
                dataReplica2023.getLogHighWatermark(),
                dataReplica2024.getLogHighWatermark());
        ...
        // partition2023 = "2023" -> January 1, 2023 00:00:00
        long expectedTimestamp2023 = parsePartitionStringToMillis("2023");
        // partition2024 = "2024" -> January 1, 2024 00:00:00
        long expectedTimestamp2024 = parsePartitionStringToMillis("2024");
        ...
        // Wait for index replication to complete by checking IndexApplier status
        // This is more reliable than Thread.sleep
        final long finalExpectedTimestamp2023 = expectedTimestamp2023;
        final long finalExpectedTimestamp2024 = expectedTimestamp2024;
                ...
                () -> {
                    ...
                    for (int bucketId = 0; bucketId < indexBucketCount; bucketId++) {
                        TableBucket idxBucket =
                                new TableBucket(idxNameTableInfo.getTableId(), bucketId);
                        int leader = FLUSS_CLUSTER_EXTENSION.waitAndGetLeader(idxBucket);
                        ReplicaManager replicaManager =
                                FLUSS_CLUSTER_EXTENSION
                        ...
                        totalRecords += kvValues.size();
                    }
                    assertThat(totalRecords)
                            .withFailMessage(
                                    "Expected 4 records in index table, but found %d", totalRecords)
                            .isEqualTo(4);
                });

        // Now verify the timestamps
        ...
        for (int bucketId = 0; bucketId < indexBucketCount; bucketId++) {
            ...
            for (byte[] valueBytes : kvValues) {
                // Directly read timestamp from value bytes for verification
                // Format: [timestamp(8)][schemaId(2)][row data]
                ...
                assertThat(timestamp)
                        .as(
                                ...
                                bucketId, finalExpectedTimestamp2023, finalExpectedTimestamp2024)
                        .isIn(finalExpectedTimestamp2023, finalExpectedTimestamp2024);

                totalRecordsWithValidTimestamp++;

                LOG.info("Bucket {}: Found record with timestamp {}", bucketId, timestamp);
            }
        }
    ...
    /**
     * Helper method to parse partition string to milliseconds timestamp. Uses system default time
     * zone to match the actual behavior of auto-partition timestamp calculation.
     *
    ...
    private long parsePartitionStringToMillis(String partitionString) {
        try {
            // Parse "yyyy" format to epoch milliseconds using system default time zone
            // This matches the default behavior of TABLE_AUTO_PARTITION_TIMEZONE
            java.time.format.DateTimeFormatter formatter =
                    java.time.format.DateTimeFormatter.ofPattern("yyyy");
            int year = Integer.parseInt(partitionString);
            java.time.LocalDate localDate = java.time.LocalDate.of(year, 1, 1);
            return localDate
