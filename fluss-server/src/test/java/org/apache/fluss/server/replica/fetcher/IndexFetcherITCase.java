...
public class IndexFetcherITCase {
    ...
    void testPartitionedTableIndexTimestampAssignment() throws Exception {
        ...
        // Create TsValueDecoder with SchemaGetter for schema evolution support
        TsValueDecoder tsValueDecoder =
                new TsValueDecoder(
                        new org.apache.fluss.metadata.SchemaGetter() {
                            @Override
                            public org.apache.fluss.metadata.Schema getSchema(int schemaId) {
                                return idxNameTableInfo.getSchema();
                            }

                            @Override
                            public CompletableFuture<org.apache.fluss.metadata.SchemaInfo>
                                    getSchemaInfoAsync(int schemaId) {
                                return CompletableFuture.completedFuture(
                                        idxNameTableInfo.getSchemaInfo());
                            }

                            @Override
                            public org.apache.fluss.metadata.SchemaInfo getLatestSchemaInfo() {
                                return idxNameTableInfo.getSchemaInfo();
                            }

                            @Override
                            public void release() {
                                // No resources to release in test
                            }
                        },
                        KvFormat.INDEXED);
