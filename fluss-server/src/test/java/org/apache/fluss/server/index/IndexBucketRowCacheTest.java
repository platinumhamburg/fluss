...
class IndexBucketRowCacheTest {
    ...
    /** Test memory reclaim under high concurrency and stress conditions. */
    @Test
    void testConcurrentMemoryReclaimStress() throws Exception {
        ...
        try {
            ...
            // Writer threads: continuously write data
                                ...
                                () -> {
                                    try {
                                        ...
                                        for (int j = 0; j < operationsPerThread; j++) {
                                            long offset = threadId * 1000L + j;
                                            int dataIndex =
                                                    (int) (offset % INDEX_CACHE_DATA.size());
                                            IndexedRow testRow = createTestIndexedRow(dataIndex);
                                            cache.writeIndexedRow(
                                                    offset, ChangeType.INSERT, testRow);
            ...
            // Cleanup threads: continuously perform garbage collection
                                ...
                                () -> {
                                    try {
                                        ...
                                        for (int j = 0; j < operationsPerThread / 2; j++) {
                                            // Cleanup with increasing horizon
                                            long horizon = j * 50L;
            ...
            // Final cleanup to verify system stability
            cache.garbageCollectBelowOffset(Long.MAX_VALUE);

            // Verify memory is properly reclaimed
            CommonTestUtils.waitUntil(
                    () -> cache.getMemorySegmentCount() <= 1,
                    Duration.ofSeconds(10),
                    "Memory should be reclaimed after complete cleanup");
    ...
    /** Test memory reclaim behavior when memory pool is under pressure. */
    @Test
    void testMemoryReclaimUnderPoolPressure() throws Exception {
        // Create a smaller memory pool to simulate pressure
        Configuration conf = new Configuration();
        conf.set(ConfigOptions.SERVER_INDEX_CACHE_MEMORY_SIZE, MemorySize.parse("100mb"));
        conf.set(ConfigOptions.SERVER_INDEX_CACHE_PAGE_SIZE, MemorySize.parse("32kb"));
        conf.set(ConfigOptions.SERVER_INDEX_CACHE_PER_REQUEST_MEMORY_SIZE, MemorySize.parse("1mb"));
        LazyMemorySegmentPool smallPool = LazyMemorySegmentPool.createIndexCacheBufferPool(conf);
        IndexBucketRowCache pressureCache = new IndexBucketRowCache(smallPool);

        try {
            // Fill up the memory pool close to capacity
            ...
            for (Long offset : offsets) {
                IndexedRow testRow = createTestIndexedRow((int) (offset % INDEX_CACHE_DATA.size()));
                try {
                    pressureCache.writeIndexedRow(offset, ChangeType.INSERT, testRow);
                } catch (Exception e) {
            ...
            pressureCache.garbageCollectBelowOffset(25L);

            // Should be able to write more data after cleanup
            for (long i = 100; i < 110; i++) {
                IndexedRow testRow = createTestIndexedRow((int) (i % INDEX_CACHE_DATA.size()));
                pressureCache.writeIndexedRow(i, ChangeType.INSERT, testRow);
            }

            // Verify that new data is readable
            for (long i = 100; i < 110; i++) {
                LogRecords result = pressureCache.readIndexLogRecords(i, i + 1);
                assertThat(result.sizeInBytes()).isGreaterThan(0);
            }
    ...
    private void writeTestDataAtOffset(long offset) throws Exception {
        ...
        IndexedRow testRow = createTestIndexedRow(dataIndex);
        cache.writeIndexedRow(offset, ChangeType.INSERT, testRow);
    }
