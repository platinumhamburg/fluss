...
public class PartitionedIndexTableITCase {
    ...
    void testPartitionOperationsWithDataWrites() throws Exception {
        ...
        TablePath nameIndexPath = TablePath.forIndexTable(tablePath, "idx_name");
    ...
    void testPartitionedTableTTLTimestampPropagation() throws Exception {
        ...
        TablePath singleIndexPath = TablePath.forIndexTable(singlePartitionPath, "idx_name");
        ...
        String[] singlePartitionDates = {
            java.time.LocalDate.now().plusDays(100).format(dtFmt),
            java.time.LocalDate.now().plusDays(110).format(dtFmt),
            java.time.LocalDate.now().plusDays(120).format(dtFmt)
        };
        ...
        TablePath multiIndexPath = TablePath.forIndexTable(multiPartitionPath, "idx_name");
        ...
        String[][] multiPartitions = {
            {"us-east", java.time.LocalDate.now().plusMonths(10).format(ymFmt)},
            {"us-west", java.time.LocalDate.now().plusMonths(11).format(ymFmt)},
            {"eu-west", java.time.LocalDate.now().plusMonths(12).format(ymFmt)}
        };
    ...
    private long createPartitionAndGetId(
        ...
        PartitionRegistration partitionRegistration =
                zkClient.getPartition(tablePath, partitionName).get();
        return partitionRegistration.getPartitionId();
    }
}
