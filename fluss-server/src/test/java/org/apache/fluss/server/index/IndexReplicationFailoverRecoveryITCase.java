...
class IndexReplicationFailoverRecoveryITCase {
    ...
    private static Configuration initConfig() {
        ...
        // Small batch size to force multi-batch cold loading in tests
        conf.set(ConfigOptions.SERVER_INDEX_CACHE_COLD_LOAD_BATCH_SIZE, MemorySize.parse("2kb"));
        // Tighten timeouts for faster failover detection in tests
    ...
    void testIndexApplierStateRecoveryAfterLeaderSwitch() throws Exception {
        ...
        List<Tuple2<Object[], Object[]>> firstBatch = createTestBatch(1, "alice", "bob", "charlie");
        ...
        // Wait for data table HW to advance (IndexDataProducer needs committed records)
        ...
        retry(
                ...
                () -> {
                    ...
                    assertThat(dataTableHW).isGreaterThanOrEqualTo(3);
                });
        ...
        zkClient.updateLeaderAndIsr(
                idxNameBucket, newLeaderAndIsr, ZkVersion.MATCH_ANY_VERSION.getVersion());
        ...
        List<Tuple2<Object[], Object[]>> secondBatch =
                createTestBatch(4, "david", "eve", "frank", "grace", "henry", "iris", "jack");
        ...
        retry(
                ...
                () -> {
                    long dataTableHW = dataTableLeaderReplica.getLogHighWatermark();
                    assertThat(dataTableHW).isGreaterThanOrEqualTo(10);
                });
        ...
        retry(
                ...
                () -> {
                    ...
                    assertThat(currentLogEndOffset).isGreaterThan(logEndOffsetBeforeSwitch);
                });
    ...
    void testDataBucketLeaderChangeNotification() throws Exception {
        ...
        List<Tuple2<Object[], Object[]>> secondBatch =
                createTestBatch(
                        4, "david", "eve", "frank", "grace", "henry", "iris", "jack", "kate", "leo",
                        "mary", "nick", "olivia", "paul", "queen", "rose", "sam", "tim");
        KvRecordBatch kvRecords2 =
                genKvRecordBatch(INDEXED_KEY_TYPE, INDEXED_ROW_TYPE, secondBatch);
        newDataTableGateway
                .putKv(newPutKvRequest(dataTableId, dataTableBucket.getBucket(), -1, kvRecords2))
        ...
        retry(
                ...
                () -> {
                    ...
                    assertThat(dataTableHW).isGreaterThanOrEqualTo(20);
                });
        ...
        retry(
                ...
                () -> {
                    ...
                    assertThat(currentDataEndOffset).isGreaterThan(finalDataEndOffsetBeforeSwitch);
                    ...
                    assertThat(currentIndexEndOffset).isEqualTo(expectedIndexEndOffset);
                });
    }

    /**
     * Test cold data loading when IndexReplica is stopped and restarted. Switches DataBucket leader
     * to force cold data path (WAL loading instead of hot cache).
     */
    @Test
    void testColdDataLoadingWhenIndexReplicaFailover() throws Exception {
        ...
        zkClient.updateLeaderAndIsr(
                idxNameBucket, stoppedLeaderAndIsr, ZkVersion.MATCH_ANY_VERSION.getVersion());
        ...
        retry(
                ...
                () -> {
                    ...
                    assertThat(dataTableHW).isGreaterThanOrEqualTo(recordCount);
                });
        ...
        assertThat(newLeaderLogEndOffset).isEqualTo(dataLogEndOffset);
        ...
        retry(
                ...
                () -> {
                    ...
                    assertThat(newLeaderHW).isGreaterThanOrEqualTo(dataLogEndOffset);
                });
        ...
        zkClient.updateLeaderAndIsr(
                idxNameBucket, restoredLeaderAndIsr, ZkVersion.MATCH_ANY_VERSION.getVersion());
        ...
        retry(
                ...
                () -> {
                    ...
                    assertThat(currentDataEndOffset).isEqualTo(currentDataLogEndOffset);
                });
        ...
        retry(
                ...
                () -> {
                    ...
                    LOG.info(
                            "Enhanced verification - LastApplyDataEndOffset: {}, IndexBucket LogEndOffset: {}, Data LogEndOffset: {}",
                    ...
                    assertThat(lastApplyDataEndOffset).isGreaterThan(0);
                });
    ...
    void testCascadingFailoverMultipleSequentialLeaderSwitches() throws Exception {
        ...
        LeaderAndIsr newLeaderAndIsr1 =
                new LeaderAndIsr(
                        ...
                        bucketEpoch1);
        zkClient.updateLeaderAndIsr(
                idxNameBucket, newLeaderAndIsr1, ZkVersion.MATCH_ANY_VERSION.getVersion());
        ...
        LeaderAndIsr newLeaderAndIsr2 =
                new LeaderAndIsr(
                        ...
                        bucketEpoch2);
        zkClient.updateLeaderAndIsr(
                idxNameBucket, newLeaderAndIsr2, ZkVersion.MATCH_ANY_VERSION.getVersion());
    ...
    void testIndexCommitDataOffsetConsistency() throws Exception {
        ...
        // Step 2: Verify lastApplyRecordsDataEndOffset > 0
        TabletServer idxLeaderServer = FLUSS_CLUSTER_EXTENSION.getTabletServerById(idxLeader);
        retry(
                Duration.ofMinutes(1),
                () -> {
                    ...
                    assertThat(status.getLastApplyRecordsDataEndOffset()).isGreaterThanOrEqualTo(0);
                });
        ...
        LOG.info("Before switch: applyOffset={}", applyOffsetBefore);
    ...
    private static List<Tuple2<Object[], Object[]>> createTestBatch(int startId, String... names) {
        List<Tuple2<Object[], Object[]>> batch = new ArrayList<>();
        for (int i = 0; i < names.length; i++) {
            int id = startId + i;
            String name = names[i];
            batch.add(Tuple2.of(new Object[] {id}, new Object[] {id, name, name + "@example.com"}));
        }
        return batch;
    }
