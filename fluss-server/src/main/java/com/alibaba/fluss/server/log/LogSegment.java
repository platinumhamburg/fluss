...
public final class LogSegment {
    ...
    private FetchDataInfo readWithFilter(
        ...
        if (!iter.hasNext()) {
            LOG.info(
                    "No batches found after filtering for startOffset {} at position {} in segment {}, falling back to unfiltered iterator",
                    startOffset,
                    startPosition,
                    this);
            iter = fileLogRecords.batchIterator(startOffset, startPosition);
            if (!iter.hasNext()) {
                LOG.info(
                        "No batches found even without filtering for startOffset {} at position {} in segment {}",
                        startOffset,
                        startPosition,
                        this);
                return null;
        ...
        if (firstBatch.position() <= maxPosition) {
            if (null == projection) {
                builder.addBytes(firstBatch.getBytesView());
