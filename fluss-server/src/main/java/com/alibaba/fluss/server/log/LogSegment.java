...
public final class LogSegment {
     ...
     * @param recordBatchFilter The filter to apply to the log records
     * @return The fetched data and the offset metadata of the first message whose offset is >=
     *     startOffset, or null if the startOffset is larger than the largest offset in this log
     */
    @Nullable
    public FetchDataInfo read(
        ...
        return readWithFilter(
                startOffset, maxSize, maxPosition, minOneMessage, projection, recordBatchFilter);
    }
    ...
    private FetchDataInfo readWithFilter(
        ...
        Iterator<FileChannelLogRecordBatch> iter =
                fileLogRecords.batchIterator(startOffset, 0).filter(recordBatchFilter);
        if (!iter.hasNext()) {
            return null;
        }
        int startPosition = -1;
        ...
        while (iter.hasNext()) {
            ...
            if (startPosition == -1) {
                startPosition = batch.position();
            }

            if (batch.position() > maxPosition) {
                break;
            }
            if (batch.sizeInBytes() > maxSize) {
