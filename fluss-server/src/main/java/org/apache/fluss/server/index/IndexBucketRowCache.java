...
public class IndexBucketRowCache implements Closeable {
        ...
        return inReadLock(
                ...
                () -> {
                    ...
                    return this.offsetRanges
                            ...
                            .values()
                            .stream()
                            .map(r -> r.getEntryCountInRange(startOffset, maxEndOffset))
                            .reduce(Integer::sum)
                            .orElse(0);
                });
    }
    ...
    private static class OffsetRange implements Closeable {
        ...
        public int getEntryCountInRange(long startOffset, long endOffset) {
            return localIndex.getEntriesInRange(startOffset, endOffset).size();
        }
        ...
        public int writeIndexedRow(long logOffset, ChangeType changeType, IndexedRow row)
            ...
            RowCacheIndexEntry entry =
                    new RowCacheIndexEntry(targetPage, segmentOffset, sizeInBytes);
    ...
    public static final class RowCacheIndex {
        ...
        public Set<OffsetRange.CachePage> removeEntriesBelowHorizon(long horizon) {
            ...
            RowCacheIndexEntry lastEntry = indexEntriesToRemove.lastEntry().getValue();
            int lastEntryEndOffset = lastEntry.segmentOffset + lastEntry.rowLength;
            Set<OffsetRange.CachePage> pagesToReclaim =
                    indexEntriesToRemove.values().stream()
                            .map(RowCacheIndexEntry::getPage)
                            .filter(page -> page.getNextOffset() <= lastEntryEndOffset)
                            .collect(Collectors.toSet());
            indexEntriesToRemove.clear();
            return pagesToReclaim;
        }
     ...
     * - rowLength: The length of the serialized IndexedRow data
     */
    @Internal
    public static final class RowCacheIndexEntry {
         ...
         */
        public RowCacheIndexEntry(OffsetRange.CachePage page, int segmentOffset, int rowLength) {
            if (segmentOffset < 0) {
                throw new IllegalArgumentException(
