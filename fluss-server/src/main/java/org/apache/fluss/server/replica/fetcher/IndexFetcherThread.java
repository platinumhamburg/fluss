...
final class IndexFetcherThread extends ShutdownableThread {
    ...
    private void delayIndexBuckets(Set<DataIndexTableBucket> indexBuckets, long delay)
            throws InterruptedException {
        indexBucketStatusMapLock.lockInterruptibly();
        try {
    ...
    private void handleNotLeaderOrFollowerError(
            ...
            DataBucketIndexFetchStatus currentIndexFetchStatus) {
        try {
            delayIndexBuckets(Collections.singleton(dataIndexBucket), fetchBackOffMs);
        } catch (InterruptedException e) {
            throw new RuntimeException(e);
        }
    }
    ...
    private void handleFetchIndexLogResponseOfSuccessBucket(
        ...
        try {
            ...
            if (startOffset != currentAppliedEndOffset) {
                LOG.warn(
                        "Index fetch offset mismatch for data bucket {} -> index bucket {}: expected start={}, got range [{}, {}), retrying after delay",
                        dataBucket,
                        indexBucket,
                ...
                LOG.debug(
                        "No new index data available for data bucket {} -> index bucket {} at offset {}, retrying after delay",
                        dataBucket,
                        indexBucket,
                        startOffset);
                delayIndexBuckets(Collections.singleton(dataIndexBucket), fetchBackOffMs);
            ...
            LOG.error(
                    "Error while processing index data for data-index bucket {}, delayed for retry.",
                    dataIndexBucket,
                    e);
            try {
                delayIndexBuckets(Collections.singleton(dataIndexBucket), fetchBackOffMs);
            } catch (InterruptedException ex) {
                LOG.error("Interrupted while handle index replica with error.", ex);
            }
        }
    }
    ...
    private void handleIndexBucketWithError(Set<DataIndexTableBucket> indexBuckets) {
        if (!indexBuckets.isEmpty()) {
            LOG.info(
                    "Index fetch failed for {} buckets, retrying after delay: {}",
                    indexBuckets.size(),
                    indexBuckets);
            try {
                delayIndexBuckets(indexBuckets, fetchBackOffMs);
            } catch (InterruptedException e) {
                LOG.error("Interrupted while handle index replica with error.", e);
            }
        }
    }
    ...
    private long processFetchIndexResult(
            DataIndexTableBucket dataIndexBucket,
            long fetchOffset,
            FetchIndexLogResultForBucket indexResultData)
            throws Exception {
        // Get the index table replica and data bucket for applying index data
        TableBucket indexBucket = dataIndexBucket.getIndexBucket();
        TableBucket dataBucket = dataIndexBucket.getDataBucket();
        Replica indexReplica = replicaManager.getReplicaOrException(indexBucket);
        ...
        try {
            serverMetricGroup.indexApplyRequests().inc();

            // Get IndexApplier and current state first
            IndexApplier indexApplier = indexReplica.getIndexApplier();
            if (indexApplier == null) {
                // IndexApplier is required for index replication
                String errorMsg =
                        String.format(
                                "IndexApplier is not available for index bucket %s. "
                                        + "Index replication requires IndexApplier for proper state management.",
                                indexBucket);
                LOG.error(errorMsg);
            ...
            // Check if this is a valid fetch based on WAL address space progression
            long startOffset = indexResultData.getStartOffset();
            long endOffset = indexResultData.getEndOffset();
            boolean hasValidFetch =
                    endOffset > currentAppliedEndOffset && startOffset <= currentAppliedEndOffset;
            ...
            if (hasValidFetch) {
                LOG.trace(
                        "Index replica processing fetch for WAL range [{}, {}) from data bucket {} to index bucket {}, {} bytes of index data",
                        startOffset,
                        endOffset,
                        dataBucket,
                        indexBucket,
                ...
                // Use IndexApplier for proper index data application
                try {
                    // Apply index records directly through IndexApplier with WAL address space
                    // range
                    // Even if indexRecords is empty, we need to advance IndexApplier's state
                    ...
                    LOG.trace(
                            "Applied index records from data bucket {} to index bucket {} through IndexApplier, "
                                    + "WAL range [{}, {}), {} bytes data, new applied offset: {}",
                            dataBucket,
                            indexBucket,
                            startOffset,
                            endOffset,
                    ...
                    // Always return currentAppliedEndOffset + 1 as next fetch offset
                    return newAppliedOffset + 1;
                } catch (Exception e) {
                    // Log IndexApplier-specific errors and increment error counter
                    serverMetricGroup.indexApplyErrors().inc();
                    LOG.warn(
                            "Error applying index records through IndexApplier for data bucket {} to index bucket {}: {}",
                            dataBucket,
                            indexBucket,
                            e.getMessage(),
                            e);
                ...
                // No valid fetch: either no progression (endOffset <= currentAppliedEndOffset)
                // or there's a gap (startOffset > currentAppliedEndOffset)
                if (startOffset > currentAppliedEndOffset) {
                    // Gap detected - this should not happen in normal index replication
                    LOG.warn(
                            "Gap detected in index fetch for data bucket {} to index bucket {}: "
                                    + "startOffset {} > current applied offset {}. This may indicate data loss or out-of-order delivery.",
                            dataBucket,
                            indexBucket,
                            startOffset,
                            currentAppliedEndOffset);
                } else {
                    // No progression: endOffset <= currentAppliedEndOffset
                    LOG.trace(
                            "No progression for data bucket {} to index bucket {}: "
                                    + "endOffset {} <= current applied offset {}",
                            dataBucket,
                            indexBucket,
                            endOffset,
                            currentAppliedEndOffset);
                ...
                // Always return currentAppliedEndOffset + 1 as next fetch offset
                long nextFetchOffset = currentAppliedEndOffset + 1;
                LOG.trace(
                        "Returning next fetch offset {} for data bucket {} to index bucket {}",
                        nextFetchOffset,
                        dataBucket,
                        indexBucket);
            ...
            serverMetricGroup.indexApplyLatencyHistogram().update(latencyNs / 1_000_000);
        }
    }
