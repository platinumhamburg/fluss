...
class IndexFetcherTarget {
    ...
    /** Invalid data bucket leader server ID marker. */
    static final int INVALID_DATA_BUCKET_LEADER_ID = -1;
    ...
    /**
     * The last known leader server ID of the data bucket that this index bucket fetches from. This
     * is the tablet server hosting the data bucket leader, NOT the index bucket's own leader.
     * Updated by IndexFetcherManager during reconciliation when leader discovery succeeds.
     */
    private volatile int lastKnownDataBucketLeaderId = INVALID_DATA_BUCKET_LEADER_ID;
    ...
    /**
     * Returns the last known leader server ID of the data bucket. This is the tablet server that
     * hosts the data bucket leader replica, from which the index fetcher pulls index data.
     */
    int getLastKnownDataBucketLeaderId() {
        return lastKnownDataBucketLeaderId;
    }
    ...
    void setLastKnownDataBucketLeaderId(int serverId) {
        this.lastKnownDataBucketLeaderId = serverId;
    }
    ...
    public String toString() {
        return String.format(
                "IndexFetcherTarget{bucket=%s, dataBucketLeader=%s, state=%s, failures=%d, reason='%s'}",
                dataIndexTableBucket,
                lastKnownDataBucketLeaderId >= 0 ? lastKnownDataBucketLeaderId : "unknown",
                state,
                failureCount,
                lastFailureReason);
    }
