...
class IndexFetcherTarget {
    ...
    /**
     * Check if this goal is ready for retry based on simple fixed backoff strategy.
     *
     * <p>The backoff strategy uses a simple fixed minimum interval to prevent retry storms:
     *
     * <ul>
     *   <li>PENDING state: No backoff, can be processed immediately to allow quick startup
     *   <li>FAILED state: Fixed minimum backoff of 5 seconds (regardless of failure count)
     * </ul>
     *
     * <p>This simple approach ensures that once a target fails, it won't be retried for at least 5
     * seconds, which is sufficient to prevent retry storms during leader failover scenarios. The
     * fixed interval avoids complexity and provides predictable behavior.
     */
    public boolean isReadyForRetry(long currentTimeMs, long baseBackoffMs) {
        ...
        // PENDING state: ready immediately (no backoff needed for first attempt)
        if (state == State.PENDING) {
            return true;
        }

        // FAILED state: use fixed minimum backoff of 5 seconds
        // This ensures backoff >= reconciliation interval (5s) to prevent retry storms
        long minBackoffMs = 5000L;
        return currentTimeMs - lastAttemptTimestamp >= minBackoffMs;
    }
