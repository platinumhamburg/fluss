 ...
 *
 * <p>This service manages column locks at the table/partition level. All buckets in the same
 * table/partition share the same lock resources, greatly simplifying the architecture.
 *
 * <p>Key features:
 *
 * <ul>
 *   <li>Table/Partition-level locks: Simplified lock granularity
...
public class ServerColumnLockService {
    ...
    // Lock resources: TablePartition -> owner locks
    private final Map<TablePartition, Map<String, ColumnLockInfo>> lockResources;
    ...
    /** Represents a lock request at table/partition level. */
    public static class PartitionLockRequest {
    ...
    /** Represents the result of a lock request at table/partition level. */
    public static class PartitionLockResult {
        private final TablePartition tablePartition;
        private final long acquiredTime; // -1 if failed
        private final @Nullable String errorMessage;

        private PartitionLockResult(
                TablePartition tablePartition, long acquiredTime, @Nullable String errorMessage) {
            this.tablePartition = tablePartition;
            this.acquiredTime = acquiredTime;
            this.errorMessage = errorMessage;
        }

        public static PartitionLockResult success(
                TablePartition tablePartition, long acquiredTime) {
            return new PartitionLockResult(tablePartition, acquiredTime, null);
        }

        public static PartitionLockResult failure(
                TablePartition tablePartition, String errorMessage) {
            return new PartitionLockResult(tablePartition, -1, errorMessage);
        }
    ...
    /**
     * Attempt to acquire column lock at table/partition level.
     *
     * @param request lock request for table/partition
     * @return result of the lock acquisition
     */
    public PartitionLockResult acquireLock(PartitionLockRequest request) {
        if (closed) {
            LOG.warn("ServerColumnLockService is closed, rejecting lock acquisition");
            return PartitionLockResult.failure(request.getTablePartition(), "Service is closed");
        }

        TablePartition tablePartition = request.getTablePartition();
        long acquiredTime =
                acquireLock(
                        tablePartition,
                        request.getSchemaId(),
        ...
        if (acquiredTime < 0) {
            String errorMsg =
                    String.format(
                            "Column lock conflict: Failed to acquire lock for %s", tablePartition);
            LOG.warn(errorMsg);
            return PartitionLockResult.failure(tablePartition, errorMsg);
        } else {
            LOG.info("Successfully acquired lock for {}", tablePartition);
            return PartitionLockResult.success(tablePartition, acquiredTime);
        }
    }

    /**
     * Release column lock at table/partition level.
     *
     * @param tablePartition the table/partition to release lock for
     * @param ownerId the owner ID of the lock
     * @return result of the lock release
     */
    public PartitionLockResult releaseLockForPartition(
            TablePartition tablePartition, String ownerId) {
        releaseLock(tablePartition, ownerId);
        long timestamp = System.currentTimeMillis();
        LOG.info("Released lock for {}, owner: {}", tablePartition, ownerId);
        return PartitionLockResult.success(tablePartition, timestamp);
    }

    /**
     * Renew a column lock at table/partition level.
     *
     * @param tablePartition the table/partition
     * @param ownerId the lock owner ID
     * @return result of the lock renewal
     */
    public PartitionLockResult renewLockForPartition(
            TablePartition tablePartition, String ownerId) {
        if (closed) {
            LOG.warn("ServerColumnLockService is closed, rejecting lock renewal");
            return PartitionLockResult.failure(tablePartition, "Service is closed");
        }

        long renewedTime = renewLock(tablePartition, ownerId);
        if (renewedTime < 0) {
            return PartitionLockResult.failure(tablePartition, "Lock not found or expired");
        }
        return PartitionLockResult.success(tablePartition, renewedTime);
    }
    ...
    public boolean validateWritePermission(
        ...
        return validateWritePermission(tablePartition, ownerId, schemaId, targetColumns);
    }
    ...
    public boolean hasActiveLocks(TableBucket tableBucket) {
        ...
        Map<String, ColumnLockInfo> locks = lockResources.get(tablePartition);
        return locks != null && !locks.isEmpty();
    }

    // ================== Internal Lock Management Methods ==================

    private synchronized long acquireLock(
            TablePartition tablePartition,
            int schemaId,
            @Nullable int[] columnIndexes,
            String ownerId,
        ...
        // Get or create lock map for this resource
        Map<String, ColumnLockInfo> locks =
                lockResources.computeIfAbsent(tablePartition, k -> MapUtils.newConcurrentHashMap());
        ...
        if (existingLock != null) {
            if (existingLock.isExpired(currentTime)) {
                LOG.info(
                        "[Server-{}] Removing expired lock for {}, owner: {}",
                        serverId,
                        tablePartition,
                        ownerId);
                ...
                if (existingLock.getSchemaId() != schemaId
                    ...
                    LOG.warn(
                            "[Server-{}] Lock re-acquisition conflict for {}, owner: {}. Existing lock (schema={}, columns={}), requested (schema={}, columns={})",
                            serverId,
                            tablePartition,
                            ownerId,
                ...
                LOG.debug(
                        "[Server-{}] Re-acquired lock for {}, owner: {}",
                        serverId,
                        tablePartition,
                        ownerId);
                return currentTime;
        ...
        while (iterator.hasNext()) {
            ...
            if (!entry.getKey().equals(ownerId)) {
                ...
                if (otherLock.isExpired(currentTime)) {
                    ...
                    LOG.warn(
                            "[Server-{}] Lock conflict for {}, existing owner: {} (schema={}, columns={}), requested owner: {} (schema={}, columns={})",
                            serverId,
                            tablePartition,
                            entry.getKey(),
        ...
        LOG.info(
                "[Server-{}] Acquired lock for {}, owner: {} (schema={}, columns={})",
                serverId,
                tablePartition,
                ownerId,
                schemaId,
    ...
    private synchronized boolean releaseLock(TablePartition tablePartition, String ownerId) {
        Map<String, ColumnLockInfo> locks = lockResources.get(tablePartition);
        if (locks == null) {
            return true;
        }
        ...
        if (locks.isEmpty()) {
            lockResources.remove(tablePartition);
        }

        if (lock != null) {
            LOG.info(
                    "[Server-{}] Released lock for {}, owner: {}",
                    serverId,
                    tablePartition,
                    ownerId);
        }
    ...
    private synchronized long renewLock(TablePartition tablePartition, String ownerId) {
        if (closed) {
            return -1;
        }
        ...
        if (locks == null) {
            LOG.warn(
                    "[Server-{}] Lock resource not found for {}, owner: {}",
                    serverId,
                    tablePartition,
                    ownerId);
            return -1;
        ...
        if (lock == null) {
            LOG.warn(
                    "[Server-{}] Lock not found for {}, owner: {}",
                    serverId,
                    tablePartition,
                    ownerId);
            return -1;
        ...
        LOG.debug("[Server-{}] Renewed lock for {}, owner: {}", serverId, tablePartition, ownerId);
        return currentTime;
    }

    private synchronized boolean validateWritePermission(
            TablePartition tablePartition,
            @Nullable String ownerId,
            int schemaId,
            @Nullable int[] targetColumns) {
