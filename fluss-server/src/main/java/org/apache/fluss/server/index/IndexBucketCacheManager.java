 ...
 *
 * <p>Thread Safety: Write operations (writeIndexedRowToTargetBucket, synchronizeAllBucketsToOffset)
 * must be externally serialized per index bucket. Read operations (readIndexLogRecords,
 * getCachedBytesOfIndexBucketInRange) and updateCommitOffset are safe for concurrent access because
 * IndexBucketCache uses internal ReadWriteLock.
 */
@Internal
public final class IndexBucketCacheManager implements Closeable {
    ...
    /** Interval (in retry attempts) between memory pressure warning logs. */
    private static final int MEMORY_PRESSURE_LOG_INTERVAL = 100;
    ...
    public Map<TableBucket, String> getBlockingBucketDiagnostics() {
        Map<TableBucket, String> diagnostics = new HashMap<>();
        ...
        for (Map.Entry<Long, RemoteFetchState[]> entry : bucketFetchStates.entrySet()) {
            ...
            for (int i = 0; i < states.length; i++) {
                if (states[i].getCommitOffset() == commitHorizon) {
                    TableBucket bucket = new TableBucket(indexTableId, i);
                    diagnostics.put(bucket, states[i].getDiagnosticString(indexTableId, i));
                }
            }
        }
        return diagnostics;
    ...
    public void writeIndexedRowToTargetBucket(
        ...
        for (int attempt = 0; attempt < maxRetries; attempt++) {
            ...
            try {
                ...
                if (attempt % MEMORY_PRESSURE_LOG_INTERVAL == 0 && LOG.isWarnEnabled()) {
                    LOG.warn(
                            "Memory pressure on index cache for index table {}, bucket {} at logOffset {}, attempt {}/{}",
                            indexTableId,
                            bucketId,
                            logOffset,
                            attempt,
        ...
        throw new IOException(
                String.format(
                        "Failed to write IndexedRow for index table %d, bucket %d at logOffset %d "
                                + "after %d retries due to sustained memory pressure",
                        indexTableId, bucketId, logOffset, maxRetries));
    }
    ...
    public ReadResult readIndexLogRecords(
        ...
        LogRecords records;
        try {
            records = bucketCache.readIndexLogRecords(startOffset, expectedEndOffset);
        } catch (IOException e) {
            throw new UncheckedIOException(
                    String.format(
                            "Failed to read index log records for index table %d, bucket %d, range [%d, %d)",
                            indexTableId, bucketId, startOffset, expectedEndOffset),
                    e);
        }
