...
public final class IndexApplier implements Closeable {
    ...
    /**
     * Get all data buckets that have index application progress tracked.
     *
     * <p>This includes all data buckets in the uncommittedApplyMap and indexCommitDataOffsetMap.
     *
     * @return a set of all tracked data buckets
     */
    public java.util.Set<TableBucket> getAllTrackedDataBuckets() {
        return inReadLock(
                lock,
                () -> {
                    java.util.Set<TableBucket> allDataBuckets = new java.util.HashSet<>();
                    allDataBuckets.addAll(uncommittedApplyMap.keySet());
                    allDataBuckets.addAll(indexCommitDataOffsetMap.keySet());
                    return allDataBuckets;
                });
    }
