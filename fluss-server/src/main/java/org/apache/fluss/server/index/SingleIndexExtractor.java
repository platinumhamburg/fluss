...
public final class SingleIndexExtractor implements Closeable {
    ...
    // Pre-computed formatters to avoid per-record allocation on hot path
    private static final DateTimeFormatter HOUR_FORMATTER = ofPattern("yyyyMMddHH");
    private static final DateTimeFormatter DAY_FORMATTER = ofPattern("yyyyMMdd");
            ...
            switch (unit) {
                case HOUR:
                    LocalDateTime hourDateTime =
                            LocalDateTime.parse(partitionString, HOUR_FORMATTER);
                    return hourDateTime.atZone(zoneId).toInstant().toEpochMilli();

                case DAY:
                    LocalDate dayDate = LocalDate.parse(partitionString, DAY_FORMATTER);
                    return dayDate.atStartOfDay(zoneId).toInstant().toEpochMilli();

                case MONTH:
                    LocalDate monthDate = LocalDate.parse(partitionString + "01", DAY_FORMATTER);
                    return monthDate.atStartOfDay(zoneId).toInstant().toEpochMilli();

                case YEAR:
                    LocalDate yearDate = LocalDate.parse(partitionString + "0101", DAY_FORMATTER);
    ...
    private boolean hasNullIndexColumn(InternalRow row) {
        for (int i = 0; i < indexFieldIndicesOfDataRow.length; i++) {
            int dataColumnIndex = indexFieldIndicesOfDataRow[i];
            if (dataColumnIndex >= 0 && fieldGetters[dataColumnIndex] != null) {
                if (fieldGetters[dataColumnIndex].getFieldOrNull(row) == null) {
                    LOG.warn(
                            "Null index column detected: indexTable={}, fieldIndex={}, dataColumnIndex={}, totalIndexFields={}, row={}",
                            indexTableId,
                            i,
                            dataColumnIndex,
                            indexFieldIndicesOfDataRow.length,
                            row);
                    return true;
                }
            }
