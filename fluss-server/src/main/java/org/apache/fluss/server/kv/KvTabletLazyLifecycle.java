...
public final class KvTabletLazyLifecycle {
    ...
    /**
     * Returns true if this tablet is in LAZY or FAILED state (RocksDB not open), used by
     * KvTablet.getRowCount() to decide whether to return cached value.
     */
    boolean isRocksDBNotOpen() {
    ...
    private void updateStateGauges(@Nullable LazyState from, LazyState to) {
        TabletServerMetricGroup mg = tablet.serverMetricGroup;
        if (mg == null) {
            return;
        }
        if (from != null) {
            switch (from) {
                case LAZY:
                    mg.kvTabletLazyCount().decrementAndGet();
                    break;
                case OPEN:
                    mg.kvTabletOpenCount().decrementAndGet();
                    break;
                case FAILED:
                    mg.kvTabletFailedCount().decrementAndGet();
                    break;
                default:
                    break;
            }
        ...
        switch (to) {
            case LAZY:
                mg.kvTabletLazyCount().incrementAndGet();
                break;
            case OPEN:
                mg.kvTabletOpenCount().incrementAndGet();
                break;
            case FAILED:
                mg.kvTabletFailedCount().incrementAndGet();
                break;
            default:
                break;
        }
    ...
    private void ensureOpen() {
        ...
        try {
            ...
            throw new KvStorageException(
                    "Interrupted waiting for KvTablet open: " + tablet.getTableBucket());
        } finally {
    ...
    private void drainPinsForDrop() {
        if (!drainPins()) {
            LOG.warn("{} drop drain timeout, force-resetting pins", tablet.getTableBucket());
            activePins.set(0);
        }
    }
