...
public class FieldProductAgg extends FieldAggregator {
    ...
    public Object retract(Object accumulator, Object inputField) {
        ...
        if (accumulator == null || inputField == null) {
            ...
            if (isZero(inputField)) {
                throw new IllegalArgumentException(
                        "Cannot retract product aggregation with zero value. Division by zero is not allowed.");
            }
            ...
            switch (fieldType.getTypeRoot()) {
                case DECIMAL:
                    ...
                    BigDecimal bigDecimal = mergeFieldDD.toBigDecimal();
                    BigDecimal bigDecimal1 = inFieldDD.toBigDecimal();
                    // Specify rounding mode to handle non-terminating decimal expansion
                    BigDecimal div =
                            bigDecimal.divide(
    ...
    private boolean isZero(Object value) {
        switch (fieldType.getTypeRoot()) {
            case DECIMAL:
                return ((Decimal) value).toBigDecimal().compareTo(BigDecimal.ZERO) == 0;
            case TINYINT:
                return (byte) value == 0;
            case SMALLINT:
