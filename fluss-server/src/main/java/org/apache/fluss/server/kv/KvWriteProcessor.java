...
import org.apache.fluss.memory.MemoryAllocation;
...
public final class KvWriteProcessor {
    ...
    public LogAppendInfo putAsLeader(
        ...
        try (MemoryAllocation allocation = memorySegmentPool.newAllocation()) {
            WalBuilder walBuilder =
                    createWalBuilder(writeContext.latestSchemaId, latestRowType, allocation);
    ...
    private WalBuilder createWalBuilder(
            int schemaId, RowType rowType, MemorySegmentPool memorySegmentPool) throws Exception {
        switch (logFormat) {
            case INDEXED:
