...
public final class DataBucketIndexFetchResult {
    ...
    public Map<TableBucket, FetchIndexLogResultForBucket> getIndexBucketResults() {
        return Collections.unmodifiableMap(indexBucketResults);
    }
    ...
    /**
     * Create an error result for all specified index buckets.
     *
     * @param indexBucketsToFetch the set of index buckets
     * @param error the error to set
     * @return a DataBucketIndexFetchResult with error status for all buckets
     */
    ...
    /**
     * Create a result with a specific status for all specified index buckets. This is used for
     * non-error statuses like NO_DATA_AVAILABLE or THROTTLED.
     *
     * @param indexBucketsToFetch the set of index buckets
     * @param status the status to set (must not be SUCCESS or ERROR)
     * @return a DataBucketIndexFetchResult with the specified status for all buckets
     */
    public static DataBucketIndexFetchResult withStatus(
            Set<TableBucket> indexBucketsToFetch, FetchIndexStatus status) {
        Map<TableBucket, FetchIndexLogResultForBucket> indexBucketResults = new HashMap<>();
        for (TableBucket indexBucket : indexBucketsToFetch) {
            indexBucketResults.put(indexBucket, new FetchIndexLogResultForBucket(status));
        }
        return new DataBucketIndexFetchResult(indexBucketResults);
    }
