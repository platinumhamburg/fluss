...
public class KvTabletSnapshotTarget implements PeriodicSnapshotManager.SnapshotTarget {
    ...
    public void handleSnapshotResult(
        ...
        // Write _TABLET_STATE to DFS before commit RPC so that recovery can read it
        writeTabletState(tabletState, snapshotResult.getSnapshotPath());

        // Construct lightweight CompletedSnapshot without TabletState fields
    ...
    /**
     * Writes the {@link TabletState} as a {@code _TABLET_STATE} file to the snapshot directory on
     * DFS. This is done before the commit RPC so that TabletServer recovery can read it on-demand.
     */
    private void writeTabletState(TabletState tabletState, FsPath snapshotPath) throws IOException {
        FsPath tabletStatePath = CompletedSnapshot.getTabletStateFilePath(snapshotPath);
        FileSystem fs = tabletStatePath.getFileSystem();
        byte[] jsonBytes = TabletStateJsonSerde.toJson(tabletState);
        try (FSDataOutputStream outStream =
                fs.create(tabletStatePath, FileSystem.WriteMode.OVERWRITE)) {
