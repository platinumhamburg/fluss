...
public class IndexFetcherManager {
    ...
    private void reconcileFetchers() {
        ...
        int failedCount = processInvalidFetchers(currentTimeMs);
        ...
        if (failedCount > 0 || actions.targetsToCreate.size() > 0) {
            LOG.info(
     ...
     */
    @GuardedBy("lock")
    private int processInvalidFetchers(long currentTimeMs) {
        int failedCount = 0;

        for (IndexFetcherThread fetcherThread : indexFetcherThreadMap.values()) {
            Map<DataIndexTableBucket, String> invalidateFetcherInfo =
                    fetcherThread.pollInvalidBuckets();
            if (!invalidateFetcherInfo.isEmpty()) {
                LOG.info(
                        "Processing {} fetch failures from fetcher thread {}",
                        invalidateFetcherInfo.size(),
                        fetcherThread.getName());
            }
            for (Map.Entry<DataIndexTableBucket, String> entry : invalidateFetcherInfo.entrySet()) {
                DataIndexTableBucket dataIndexBucket = entry.getKey();
    ...
    /**
     * Handle a target in PENDING or FAILED state. May add it to fechersToCreate if ready for
     * (re)creation.
     */
    @GuardedBy("lock")
    private void handlePendingOrFailedFetcher(
            IndexFetcherTarget target,
            Map<ServerIdAndFetcherId, List<IndexFetcherTarget>> fetchersToCreate,
            long currentTimeMs) {
        ...
        // Check if leader ID is invalid (-1 means no leader elected yet)
        if (leaderId == INVALID_LEADER_ID) {
            String reason =
                    String.format(
                            "Data bucket %s has no leader elected yet (leaderId=-1)%s",
                            target.getDataBucket(), queriedFromZK ? " (from ZooKeeper)" : "");
            if (target.getState() == IndexFetcherTarget.State.PENDING) {
                LOG.debug(reason + ", will retry");
            } else if (target.getFailureCount() > 0 && target.getFailureCount() % 20 == 0) {
                // Log warning periodically for persistent failures
                LOG.warn(
                        "Fetcher for data-index bucket {} has failed {} times due to no leader elected. Reason: {}",
        ...
        fetchersToCreate.computeIfAbsent(fetcherId, k -> new ArrayList<>()).add(target);
    }
    ...
    private void handleRunningFetcher(IndexFetcherTarget target) {
        if (hasValidLeader(target)) {
            ...
            // Check if current leader is invalid (-1 means no leader elected)
            if (currentLeader == INVALID_LEADER_ID) {
                LOG.info(
                        "Data bucket {} leader became unavailable (no leader elected, last known leader: {}), marking target as failed to trigger reconnection",
                        target.getDataBucket(),
                        lastKnownLeader);
                target.markFailed(
                        String.format(
                                "Data bucket leader became unavailable (no leader elected, last known: %d)",
                                lastKnownLeader),
                        System.currentTimeMillis());
