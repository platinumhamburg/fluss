...
public class IndexFetcherManager {
    ...
    private void reconcileFetchers() {
        ...
        if (LOG.isDebugEnabled() && (failedCount > 0 || !actions.targetsToCreate.isEmpty())) {
            LOG.debug(
                    "Reconciliation completed: processed {} failures, {} fetchers to create/update, {} active fetchers, {} total fetcher threads",
                    failedCount,
     ...
     * object containing targets to create and active fetchers.
     */
    @GuardedBy("lock")
    private ReconciliationActions classifyAndPrepareActions(long currentTimeMs) {
        ...
        for (Map.Entry<TableBucket, Map<TableBucket, IndexFetcherTarget>> entry :
            ...
            for (IndexFetcherTarget target : entry.getValue().values()) {
                ...
                switch (target.getState()) {
                    ...
                    case RUNNING:
                        handleRunningFetcher(target, activeFetchers);
                        break;
        ...
        return new ReconciliationActions(targetsToCreate, activeFetchers);
    }
    ...
    /** Handle a target in RUNNING state. Tracks it as an active fetcher. */
    @GuardedBy("lock")
    private void handleRunningFetcher(
            IndexFetcherTarget target, Set<ServerIdAndFetcherId> activeFetchers) {
        if (hasValidLeader(target)) {
            ...
            ServerIdAndFetcherId fetcherId =
                    new ServerIdAndFetcherId(
                            ...
                            getFetcherId(target.getDataBucket()));
