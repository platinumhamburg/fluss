 ...
 */
@Internal
public final class IndexBucketCache implements Closeable {
     ...
     */
    public LogRecords readIndexLogRecords(long startOffset, long maxEndOffset) throws IOException {
        return inReadLock(
                lock,
                () -> {
                    if (closed) {
    ...
    private void checkAndMergeWithNextRange(OffsetRange currentRange) {
        ...
        if (nextEntry != null) {
            ...
            if (nextRange.getStartOffset() == currentEnd) {
                // Capture values before merge for accurate logging
                long nextStart = nextRange.getStartOffset();
                long nextEnd = nextRange.getEndOffset();
                ...
                // No need to close removedRange - mergeWith already transferred ownership
            }
        }
    }
    ...
    private static class OffsetRange implements Closeable {
        ...
        private final MemorySegmentOutputView reusableOutputView =
                new MemorySegmentOutputView(MemorySegment.wrap(new byte[0]));
        private volatile boolean closed = false;
        ...
        public int writeIndexedRow(
            ...
            reusableOutputView.wrap(memorySegment);
            reusableOutputView.setPosition(segmentOffset);
            int sizeInBytes = IndexedLogRecord.writeTo(reusableOutputView, changeType, row);
        ...
        public LogRecords getRangeRecords(long start, long end, TableBucket dataBucket)
            ...
            if (recordCount == 0) {
                ...
                try (MemoryLogRecordsIndexedBuilder builder =
                        MemoryLogRecordsIndexedBuilder.builder(
                                SchemaInfo.DEFAULT_SCHEMA_ID, new ArrayList<>(), 0, false)) {
                    recordBatchBytesView = builder.build();
                }

                if (LOG.isTraceEnabled()) {
                    LOG.trace("Built empty LogRecords for range [{}, {})", start, end);
                }
            ...
            try (MemoryLogRecordsIndexedBuilder builder =
                    MemoryLogRecordsIndexedBuilder.builder(
                            SchemaInfo.DEFAULT_SCHEMA_ID, segmentBytesViews, recordCount, false)) {
                if (batchTimestamp > 0) {
        ...
        public static class CachePage {
            ...
            public boolean equals(Object o) {
                ...
                CachePage that = (CachePage) o;
                return nextOffset == that.nextOffset && segment.equals(that.segment);
            }
    ...
    public static final class RowCacheIndexEntry {
        ...
        @Override
        public int hashCode() {
            int result = System.identityHashCode(page);
