...
public class DelayedFetchIndex extends DelayedOperation {
    ...
    public DelayedFetchIndex(
        ...
        this.completeFetches = MapUtils.newConcurrentHashMap();
        this.completeFetches.putAll(completeFetches);
    ...
    private void fetchDataBucket(TableBucket dataBucket, boolean forceIncludeNotReady) {
        ...
        try {
            ...
            // Use putIfAbsent to prevent double-counting fetchedBytes when concurrent
            // tryComplete() calls race on the same data bucket.
            if (bucketResults.size() == indexRequests.size()) {
                if (completeFetches.putIfAbsent(
                                dataBucket, new DataBucketIndexFetchResult(bucketResults))
                        == null) {
            ...
            completeFetches.putIfAbsent(dataBucket, createErrorResult(indexRequests.keySet(), e));
        }
    }
    ...
    private boolean checkAndComplete() {
        if (isComplete() || fetchedBytes.get() >= params.maxBytes()) {
            if (fetchedBytes.get() >= params.maxBytes()) {
                LOG.debug(
            ...
            // Always use forceComplete() to trigger onComplete() which fires the callback.
            // Do NOT call responseCallback directly here — that would cause a double callback
            // because forceComplete() -> onComplete() also calls responseCallback.
