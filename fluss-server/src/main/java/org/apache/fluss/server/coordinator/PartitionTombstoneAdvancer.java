 ...
 * already covered by the floor). It then compacts {@code explicitSet} into the floor by absorbing
 * any contiguous run of ids starting at {@code floor + 1}. The version is always bumped by one so
 * observers can detect that <em>something</em> happened, even when the dropped partition was
 * already tombstoned.
 */
@Internal
