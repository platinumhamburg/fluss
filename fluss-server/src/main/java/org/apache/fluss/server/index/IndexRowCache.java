    ...
    public void synchronizeAllBucketsToOffset(
        ...
        try {
            ...
            for (int bucketId = 0; bucketId < indexBucketCaches.length; bucketId++) {
                ...
                // DEBUG: Log synchronization for specific buckets
                if (shouldLogDebug(bucketId)) {
                    String rangeInfoBefore = bucketCache.getOffsetRangeInfo();
                    LOG.info(
                            "[RANGE_GAP_DEBUG] IndexRowCache synchronizing: indexTableId={}, indexBucket={}, "
                                    + "targetOffset={}, batchStartOffset={}, rangesBefore={}",
                            indexTableId,
                            bucketId,
                            targetOffset,
                            batchStartOffset,
                            rangeInfoBefore.isEmpty()
                                    ? "EMPTY"
                                    : rangeInfoBefore.replace("\n", " | "));
                }
