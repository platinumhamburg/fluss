...
public abstract class WriteBatch {
    ...
    /** Result offset for callbacks (set when batch completes with result info). */
    private long resultOffset = -1;
    ...
    /** Complete the batch successfully with bucket and offset info (for KV batches). */
    public boolean complete(TableBucket bucket, long offset) {
    ...
    private void completeFutureAndFireCallbacks(@Nullable Exception exception) {
        // execute callbacks with result info (for KV batches that track offsets)
        callbacks.forEach(
                callback -> {
                    try {
                        callback.onCompletion(resultBucket, resultOffset, exception);
                    } catch (Exception e) {
                        LOG.error(
