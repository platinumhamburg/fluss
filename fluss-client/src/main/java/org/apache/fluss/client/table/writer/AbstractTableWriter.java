...
public abstract class AbstractTableWriter implements TableWriter {
    ...
    /**
     * Send a record and return a future with a result containing bucket and offset. This is used by
     * KV writers that need to track offsets for exactly-once semantics.
     *
     * @param record the record to send
     * @param resultFactory factory to create the result from bucket and offset
    ...
    protected <T> CompletableFuture<T> sendWithResult(
        ...
        writerClient.send(
                ...
                (bucket, offset, exception) -> {
                    if (exception == null) {
                        future.complete(resultFactory.create(bucket, offset));
                    } else {
                        future.completeExceptionally(exception);
                    }
                });
    ...
    /** Factory interface for creating result objects from bucket and offset. */
    @FunctionalInterface
    protected interface ResultFactory<T> {
        T create(TableBucket bucket, long offset);
    }
