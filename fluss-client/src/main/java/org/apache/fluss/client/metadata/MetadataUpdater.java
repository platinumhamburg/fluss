...
public class MetadataUpdater {
    ...
    /** Flag to reject new submissions after close() begins. */
    ...
    public void close() {
        ...
        // Reject new submissions before clearing state
        closed = true;
    ...
    private void awaitFuture(CompletableFuture<Void> future) {
        try {
            ...
            // Propagate retriable and timeout exceptions to let callers drive retry
            else if (t instanceof RetriableException) {
                throw (RetriableException) t;
            } else if (t instanceof TimeoutException) {
                throw new FlussRuntimeException("Metadata update timed out", t);
            }
            ...
            throw new FlussRuntimeException("Metadata update timed out after 60 seconds", e);
        ...
        if (serverNode == null) {
            ...
            if (serverNode == null) {
                throw new FlussRuntimeException(
                        "Leader not found after retrying "
                                + MAX_RETRY_TIMES
    ...
    public void invalidatePhysicalTableBucketMetadata(
    ...
    private CompletableFuture<Void> submitMetadataUpdateAsync(MetadataResourceKey key) {
        ...
        if (closed) {
            future.completeExceptionally(new FlussRuntimeException("MetadataUpdater is closed"));
            return future;
        }
    ...
    private Set<MetadataResourceKey> cleanupExhaustedRequests(
        ...
        for (Map.Entry<MetadataResourceKey, PendingMetadataRequest> entry : snapshot.entrySet()) {
            ...
            if (request.getRetryCount() >= MAX_METADATA_UPDATE_RETRIES) {
                ...
                pendingRequests.remove(key, request);
    ...
    private void handleNoAvailableServer(
            Map<MetadataResourceKey, PendingMetadataRequest> snapshot) {
        // Dispatch to the scheduler thread to avoid blocking the Netty IO callback thread,
        // since initializeCluster performs a blocking RPC.
        try {
            scheduler.execute(
                    () -> {
                        try {
                            ...
                            // Successfully re-initialized cluster, retry without incrementing
                            // retry count. The requests in snapshot are still in pendingRequests
                            ...
                            // handleMetadataRequestError will complete futures that exceeded
                            // max retries or keep them for retry if still under limit
            ...
            // Scheduler is shut down, fail all pending requests
            LOG.warn("Scheduler is shut down, failing pending requests", e);
            FlussRuntimeException closedException =
                    new FlussRuntimeException("MetadataUpdater is closed", e);
            for (PendingMetadataRequest request : snapshot.values()) {
                List<CompletableFuture<Void>> futures = request.snapshotAndClose();
